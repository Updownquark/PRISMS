/*
 * LogEntry.java Created Aug 3, 2011 by Andrew Butler, PSL
 */
...
public class LogEntry
{
	...
	{
		return equal(theInstanceLocation, entry.theInstanceLocation) && equal(theApp, entry.theApp)
			&& equal(theClient, entry.theClient) && equal(theSessionID, entry.theSessionID)
			&& equal(theUser, entry.theUser) && equal(theTrackingData, entry.theTrackingData)
			&& equal(theLevel, entry.theLevel) && equal(theLoggerName, entry.theLoggerName)
			&& equal(theMessage, entry.theMessage) && equal(theStackTrace, entry.theStackTrace);
	}
