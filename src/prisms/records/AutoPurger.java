...
public class AutoPurger
{
		...
		String additivityColumn) throws PrismsRecordException
	{
		...
		if(ids.length == 0)
			return;
		ChangeRecord [] records = rk.getItems(ids);
		...
		String additivityColumn) throws PrismsRecordException
	{
		...
		if(theExcludeUsers.length > 0 || theExcludeTypes.length > 0)
		{
			...
			{
				exclude.append(userColumn);
				exclude.append(" NOT IN (");
				for(int u = 0; u < theExcludeUsers.length; u++)
				{
					exclude.append(theExcludeUsers[u]);
			...
			if(theExcludeTypes.length > 0)
			{
				if(theExcludeUsers.length > 0)
					exclude.append(" AND (");
				exclude.append(subjectTypeColumn);
				exclude.append(", ");
				exclude.append(changeTypeColumn);
				exclude.append(", ");
				exclude.append(additivityColumn);
				exclude.append(") NOT IN (");
				for(int t = 0; t < theExcludeTypes.length; t++)
				{
					exclude.append('(');
					exclude.append(toSQL(theExcludeTypes[t].subjectType.name()));
					exclude.append(", ");
					exclude.append(theExcludeTypes[t].changeType == null ? "NULL"
						: toSQL(theExcludeTypes[t].changeType.name()));
					exclude.append(", ");
					exclude.append(theExcludeTypes[t].additivity < 0 ? "-"
						: (theExcludeTypes[t].additivity > 0 ? "+" : "0"));
					exclude.append(toSQL(theExcludeTypes[t].toString()));
					exclude.append(')');
		...
		long purgeTime = RecordUtils.getPurgeSafeTime(rk.getCenters());
		...
		if(age >= 0)
		{
			...
			try
			{
				rs = stmt.executeQuery(sql);
				while(rs.next())
					notSafeIDs.add(Long.valueOf(rs.getLong(1)));
			} catch(SQLException e)
			{
