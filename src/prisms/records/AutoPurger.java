...
{
	private static final Logger log = Logger.getLogger(AutoPurger.class);
	 ...
	 *         {@link #doPurge(RecordKeeper, java.sql.Statement, String, String, String, String, String, String)}
	 *         were called.
	 * @throws PrismsRecordException If an error occurs during the purge
	 */
	public int previewRowsDeleted(RecordKeeper rk, java.sql.Statement stmt, String modTable,
	 ...
	 * @throws PrismsRecordException If an error occurs during the purge
	 */
	public void doPurge(RecordKeeper rk, java.sql.Statement stmt, String modTable,
		String timeColumn, String userColumn, String subjectTypeColumn, String changeTypeColumn,
		String additivityColumn) throws PrismsRecordException
	{
	 ...
	 */
	protected long [] getPurgeIDs(RecordKeeper rk, java.sql.Statement stmt, String modTable,
		String timeColumn, String userColumn, String subjectTypeColumn, String changeTypeColumn,
		String additivityColumn) throws PrismsRecordException
	{
		...
		if(theEntryCount >= 0)
		{
			sql = "SELECT COUNT(*) FROM " + modTable + " WHERE recordNS="
				+ RecordKeeper.toSQL(rk.getNamespace());
			try
			{
		...
		long purgeTime = rk.getPurgeSafeTime();
		...
		if(age >= 0)
		{
			...
			sql += " AND recordNS=" + RecordKeeper.toSQL(rk.getNamespace());
			if(exclude != null)
				sql += " AND " + exclude;
		...
		else if(theEntryCount >= 0)
		{
			sql = "SELECT id FROM " + modTable + " WHERE recordNS="
				+ RecordKeeper.toSQL(rk.getNamespace()) + " ORDER BY " + timeColumn + " DESC";
			try
			{
				rs = stmt.executeQuery(sql);
				int count = 0;
	...
	{
		if(!(o instanceof AutoPurger))
			return false;
		AutoPurger ap = (AutoPurger) o;
