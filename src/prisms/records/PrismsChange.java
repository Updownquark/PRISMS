...
public enum PrismsChange implements SubjectType
{
	...
	public static enum CenterChange implements ChangeType
	{
		...
		/**
		 * @see PrismsCenter#getCertificates()
		 * @see PrismsCenter#setCertificates(java.security.cert.X509Certificate[])
		 */
		serverCerts(null, java.security.cert.X509Certificate[].class, false),
		/**
		...
		public String toString(int add)
		{
			...
			case serverCerts:
				return "Center Certficates Changed";
			case serverUserName:
		...
		public String toString(int add, Object major, Object minor)
		{
			...
			case serverCerts:
				return "Center " + major + "'s server certificates changed";
			case serverUserName:
		...
		public String toString(int add, Object major, Object minor, Object before, Object after)
		{
			...
			case serverCerts:
				ret += " from ";
				if(before == null || ((Object []) before).length == 0)
					ret += "none";
				else
					ret += prisms.ui.CertificateSerializer.getCertSubjectName(
						((java.security.cert.X509Certificate[]) before)[0], false);
				ret += " to ";
				if(after == null || ((Object []) after).length == 0)
					ret += "none";
				else
					ret += prisms.ui.CertificateSerializer.getCertSubjectName(
						((java.security.cert.X509Certificate[]) after)[0], false);
				return ret;
			case serverPassword:
