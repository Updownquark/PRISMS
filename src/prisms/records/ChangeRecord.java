...
public class ChangeRecord
{
	...
	/** Whether this change is local only, that is, not required for synchronization */
	 ...
	 * @param local Whether this change is local only, that is, not required for synchronization
	 ...
	 */
	public ChangeRecord(long _id, boolean local, long _time, RecordUser _user,
		SubjectType _subjectType, ChangeType _changeType, int _additivity, Object _majorSubject,
		Object _minorSubject, Object _previousValue, Object _data1, Object _data2)
	{
		id = _id;
		localOnly = local;
		...
		data2 = _data2;
		if(user == null)
			throw new IllegalArgumentException("Null user not allowed in change record");
		if(majorSubject == null)
			throw new IllegalArgumentException("Null major subject not allowed");
		if(!type.subjectType.getMajorType().isInstance(majorSubject))
			throw new IllegalArgumentException("Major subject (" + majorSubject + ", class "
				+ majorSubject.getClass().getName() + ") is not valid for subject type "
				+ type.subjectType);
		...
		if(previousValue != null)
		{
			if(type.changeType == null)
				throw new IllegalArgumentException("No previous value allowed without change type");
			if(type.changeType.getObjectType() == null)
				throw new IllegalArgumentException("No previous value allowed for change type "
					+ type.changeType);
