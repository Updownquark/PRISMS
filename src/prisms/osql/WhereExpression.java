/*
 * WhereExpression.java Created Apr 8, 2010 by Andrew Butler, PSL
 */
package prisms.osql;
...
public interface WhereExpression
{
	...
	public static final class Util
	{
		 ...
		 */
		public static <T> ValueComparison<T> valCompare(Column<T> column, CompareOperator op,
			T value) throws PrismsSqlException
		{
	...
	public static class BooleanExpression implements WhereExpression
	{
		...
		{
			for(WhereExpression exp : theChildren)
				if(!exp.isStringable())
					return false;
			return true;
		}
		...
		{
			for(WhereExpression child : theChildren)
				paramIdx = child.fillPrepared(stmt, paramIdx);
			return paramIdx;
		}
	}
	 ...
	 */
	public static abstract class ValueTest<T> implements WhereExpression
	{
	...
	public static enum CompareOperator
	{
		...
		NOT_EQUAL("<>"),
		/** Compares two values for the first being greater than the second */
		GREATER(">"),
		/** Compares two values for the first being less than the second */
		LESS("<"),
		/** Compares two values for the first being greater than or equal to the second */
		GREATER_EQUAL(">="),
		/** Compares two values for the first being less than or equal to the second */
		LESS_EQUAL("<="),
	...
	public static class ValueComparison<T> extends ValueTest<T>
	{
		 ...
		 */
		public ValueComparison(Column<T> col, CompareOperator op, T value)
			throws PrismsSqlException
		{
			...
			case NOT_EQUAL:
				break;
			case LIKE:
				if(!CharSequence.class.isAssignableFrom(col.getDataType().getJavaType()))
					throw new PrismsSqlException("Operator " + op
						+ " may only be used with a character sequence type");
				break;
			default:
				if(value instanceof String)
					throw new PrismsSqlException("Operator " + op
						+ " not valid with string data type");
			}
			theValue = value;
		}
		...
		{
			getColumn().toSQL(ret);
			if(theValue == null && !isPlaceholder)
				switch(theOperator)
				{
				case EQUAL:
					ret.append(" IS NULL");
				...
				case NOT_EQUAL:
					...
					break;
				default:
					throw new IllegalStateException("Operator " + theOperator
						+ " not valid with null value");
				}
			else
			{
				...
				if(isPlaceholder)
					...
					getColumn().getDataType().toSQL(theValue, ret, getColumn());
			}
		}

		public int fillPrepared(java.sql.PreparedStatement stmt, int paramIdx)
			throws PrismsSqlException
		{
			try
			{
	...
	public static class ValueContainment<T> extends ValueTest<T>
	{
		...
		{
			if(!theValueSet.contains(value))
				theValueSet.add(value);
		}

		/** @return The number of values or placeholders are in this containment expression */
		public int getValueCount()
		{
			return theValueSet.size() > thePlaceholders.length() ? theValueSet.size()
				: thePlaceholders.length();
		}
		...
		public boolean isStringable()
		{
			if(!thePlaceholders.isEmpty())
				return false;
			boolean ret = true;
			for(T val : theValueSet)
				if(!getColumn().getDataType().isStringable(val, getColumn()))
				{
					ret = false;
					break;
				}
		...
		public int fillPrepared(PreparedStatement stmt, int paramIdx) throws PrismsSqlException
		{
			try
			{
				for(T value : theValueSet)
					getColumn().getDataType().setParam(value, stmt, paramIdx++, getColumn());
			} catch(SQLException e)
			{
				throw new PrismsSqlException("Could not fill prepared statement: " + this, e);
			}
