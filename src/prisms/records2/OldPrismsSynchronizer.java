 ...
 *        implementation uses
 */
public class OldPrismsSynchronizer<SyncDataType extends OldSynchronizeImpl.SyncData>
{
	Logger log = Logger.getLogger(OldPrismsSynchronizer.class);

	private prisms.records2.DBRecordKeeper theKeeper;

	private OldSynchronizeImpl<SyncDataType> theImpl;
	 ...
	 */
	public OldPrismsSynchronizer(prisms.records2.DBRecordKeeper keeper,
		OldSynchronizeImpl<SyncDataType> impl)
	{
		theKeeper = keeper;
		theImpl = impl;
	}
	 ...
	 */
	public OldSynchronizeImpl<SyncDataType> getImpl()
	{
		return theImpl;
	}
		...
		try
		{
			...
			try
			{
				res = conn.getResult(thePluginName, "synchronize", "centerID",
					...
					"now", new Long(now), "syncType",
					(record == null ? syncType : record.getSyncType()).toString(), "recordID",
					new Integer(record != null ? record.getID() : 0));
			} catch(prisms.util.PrismsServiceConnector.PrismsServiceException e)
			{
		...
		throws PrismsRecordException
	{
		...
		if(lockApp)
			app.setApplicationLock(lockText, 0, 0, session);
		try
		{
	...
	ChangeRecord [] getRemoteSuccessors(ChangeRecord mod,
		prisms.records2.OldSynchronizeImpl.SyncData data)
	{
			...
			for(ChangeRecord mod : mods)
			{
				try
				{
					if(record.getCenter().getCenterID() > 0)
						theKeeper.associate(mod, record, false);
				} catch(PrismsRecordException e)
				{
	...
	public ChangeRecord [] getExportModsSince(RecordUser user, long since)
	{
		...
		try
		{
			long [] ids = theKeeper.getChangeIDs(-1, -1, since);
