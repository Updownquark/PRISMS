...
import prisms.records2.DBRecordKeeper.ChangeField;
 ...
 */
public abstract class HistoryViewer2 implements prisms.arch.AppPlugin
{
	private static final Logger log = Logger.getLogger(HistoryViewer2.class);
	...
	private prisms.records2.HistorySorter<ChangeField> theSorter;
	...
	void sendAutoPurge()
	{
		...
		evt.put("manualEnabled", new Boolean(canPurge(getUser())));
		evt.put("autoEnabled", new Boolean(canEditAutoPurge(getUser())));
		theSession.postOutgoingEvent(evt);
	...
	void setRow(ChangeRecord mod, int index)
	{
		...
		int centerID = Record2Utils.getCenterID(mod.user.getID());
		...
		if(mod instanceof ChangeRecordError)
		{
			...
			try
			{
				...
				change = Record2Utils.getChangeType(subject, error.getChangeType());
			...
			if(canShow)
			{
				ChangeRecord record = new ChangeRecord(error.id, error.time, error.user, subject,
					change, error.getAdditivity(), error.getMajorSubject(),
		...
		if(mod.type.changeType != null)
		{
			...
			if(successor.length > 0)
			{
				if(successor[0] instanceof ChangeRecordError)
					throw new IllegalStateException("Could not get next modification");
	...
	void purgeSelected()
	{
		...
		try
		{
			maxTime = Record2Utils.getPurgeSafeTime(theRecordKeeper.getCenters());
	...
	{
		final AutoPurger2 purger = new AutoPurger2();
	...
	protected Object getFieldValue(Object majorSubject, Object minorSubject, RecordType type)
	{
		...
		case center:
			...
			break;
		case autoPurge:
			AutoPurger2 purger = (AutoPurger2) majorSubject;
	 ...
	 * instance of {@link PrismsCenter} or {@link AutoPurger2}.
	 * 
	...
	protected String undo(ChangeRecord mod)
	{
		...
		case autoPurge:
			AutoPurger2 purger = (AutoPurger2) mod.majorSubject;
			if(mod.type.changeType == null)
	...
	protected String undoCenterCreated(PrismsCenter center)
	{
		try
		{
			theRecordKeeper.removeCenter(center, getUser(), null);
			return null;
		} catch(PrismsRecordException e)
		{
	 ...
	 * @return An error string, or null if the operation is successful
	 */
	protected String undoAutoPurgeUserAdded(AutoPurger2 purger, RecordUser user)
	{
	 ...
	 * @return An error string, or null if the operation is successful
	 */
	protected String undoAutoPurgeTypeAdded(AutoPurger2 purger, RecordType type)
	{
	...
	protected String undoCenterDeleted(PrismsCenter center)
	{
		try
		{
			theRecordKeeper.putCenter(center, getUser(), null);
			return null;
		} catch(PrismsRecordException e)
		{
	 ...
	 * @return An error string, or null if the operation is successful
	 */
	protected String undoAutoPurgeUserRemoved(AutoPurger2 purger, RecordUser user)
	{
	 ...
	 * @return An error string, or null if the operation is successful
	 */
	protected String undoAutoPurgeTypeRemoved(AutoPurger2 purger, RecordType type)
	{
	 ...
	 * @return An error string, or null if the operation is successful
	 */
	protected String undoAutoPurgeModified(AutoPurger2 purger, ChangeRecord mod)
	{
		boolean switchHit = false;
		switch((prisms.records2.PrismsChange.AutoPurgeChange) mod.type.changeType)
		{
		case entryCount:
			switchHit = true;
