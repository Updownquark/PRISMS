		...
		boolean withValues) throws EvaluationException
	{
		...
		if(res.isType())
			throw new EvaluationException("The operator " + theName
				+ " is not defined for type java.lang.Class", this, getStored("name").index);
		if(res.getType() == null)
			throw new EvaluationException(res.getFirstVar() + " cannot be resolved to a variable",
				this, theOperand.getMatch().index);
		if("+".equals(theName))
		{
			if(!res.getType().isPrimitive() || Boolean.TYPE.equals(res.getType()))
				throw new EvaluationException("The operator " + theName
					+ " is not defined for type " + res.typeString(), this, getStored("name").index);
			return res;
		}
		else if("-".equals(theName))
		{
			if(Double.TYPE.equals(res.getType()))
				return new EvaluationResult<Double>(Double.TYPE, withValues
					? Double.valueOf(-((Number) res.getValue()).doubleValue()) : null);
			else if(Float.TYPE.equals(res.getType()))
				return new EvaluationResult<Float>(Float.TYPE, withValues
					? Float.valueOf(-((Number) res.getValue()).floatValue()) : null);
			else if(Long.TYPE.equals(res.getType()))
				return new EvaluationResult<Long>(Long.TYPE, withValues
					? Long.valueOf(-((Number) res.getValue()).longValue()) : null);
			else if(Integer.TYPE.equals(res.getType()))
				return new EvaluationResult<Integer>(Integer.TYPE, withValues
					? Integer.valueOf(-((Number) res.getValue()).intValue()) : null);
				...
				return new EvaluationResult<Integer>(Integer.TYPE, withValues
					? Integer.valueOf(-((Number) res.getValue()).shortValue()) : null);
			else if(Byte.TYPE.equals(res.getType()))
				return new EvaluationResult<Integer>(Integer.TYPE, withValues
					? Integer.valueOf(-((Number) res.getValue()).byteValue()) : null);
			else
				throw new EvaluationException("The operator " + theName
					+ " is not defined for type " + res.typeString(), this, getStored("name").index);
		}
		...
		else if("~".equals(theName))
		{
			if(Long.TYPE.equals(res.getType()))
				return new EvaluationResult<Long>(Long.TYPE, withValues ? Long.valueOf(~((Long) res
				...
				return new EvaluationResult<Integer>(Integer.TYPE, withValues
					? Integer.valueOf(~((Short) res.getValue()).shortValue()) : null);
			else if(Byte.TYPE.equals(res.getType()))
				return new EvaluationResult<Integer>(Integer.TYPE, withValues
					? Integer.valueOf(~((Byte) res.getValue()).byteValue()) : null);
			else
				throw new EvaluationException("The operator " + theName
					+ " is not defined for type " + res.typeString(), this, getStored("name").index);
		}
