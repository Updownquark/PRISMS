...
public class ParsedSwitch extends ParsedItem
{
		...
		for(prisms.lang.ParseMatch m : match.getParsed())
		{
			if("case".equals(m.config.get("storeAs")) || "default".equals(m.config.get("storeAs")))
			{
				if(theCases.length > 0)
					theCaseBlocks = prisms.util.ArrayUtils.add(theCaseBlocks,
						caseStmts.toArray(new ParsedItem [caseStmts.size()]));
				caseStmts.clear();
				if("case".equals(m.config.get("storeAs")))
					theCases = prisms.util.ArrayUtils.add(theCases, parser.parseStructures(this, m)[0]);
				else if(hasDefault)
					throw new prisms.lang.ParseException("Only one default statement allowed in a switch", getRoot()
		...
		throws prisms.lang.EvaluationException
	{
		...
		if(withValues && var.getValue() == null)
			throw new prisms.lang.ExecutionException(new prisms.lang.Type(NullPointerException.class),
				new NullPointerException("Variable in switch statement may not be null"), this,
				theVariable.getMatch().index);
		...
		{}
			...
			throw new prisms.lang.EvaluationException(
				"Only int-convertible types, strings and enums may be used in switch statements", this,
				theVariable.getMatch().index);
			...
			if(!match || !withValues)
			{
				if(consts != null)
				{
					if(case_c instanceof ParsedIdentifier)
					{
						...
						if(!found)
							throw new prisms.lang.EvaluationException("No such constant " + id.getName() + " in enum "
								+ var.getType(), theCases[c], theCases[c].getMatch().index);
					}
					else if(case_c instanceof ParsedMethod)
					{
						...
						if(m.isMethod())
							throw new prisms.lang.EvaluationException(
								"The value in a case statement must be a constant", case_c, case_c.getMatch().index);
						...
						if(!var.getType().isAssignable(caseTypeRes.getType()))
							throw new prisms.lang.EvaluationException(
								"The value in an enum case statement must be one of the enum values", case_c,
						...
						if(!found)
							throw new prisms.lang.EvaluationException("No such constant " + m.getName() + " in enum "
								+ var.getType(), case_c, case_c.getMatch().index);
					}
					else
						throw new prisms.lang.EvaluationException("The value in a case statement must be a constant",
							this, case_c.getMatch().index);
				}
				else if(case_c instanceof ParsedString || case_c instanceof ParsedNumber
					|| case_c instanceof ParsedChar)
				{
					EvaluationResult caseRes = case_c.evaluate(env, false, withValues);
					if(!var.getType().isAssignable(caseRes.getType()))
						throw new prisms.lang.EvaluationException("type mismatch: cannot convert from "
							+ caseRes.getType() + " to " + var.getType(), case_c, case_c.getMatch().index);
					match = var.getValue().equals(caseRes.getValue());
				}
				else if(case_c instanceof ParsedNull)
					throw new prisms.lang.EvaluationException("Null not allowed for case statement", this,
						case_c.getMatch().index);
				else if(case_c instanceof ParsedBoolean)
				{
					throw new prisms.lang.EvaluationException("type mismatch: cannot convert from boolean to "
						+ var.getType(), case_c, case_c.getMatch().index);
				}
				else
				...
				for(ParsedItem pi : theCaseBlocks[c])
				{
					...
					if(withValues && res != null && res.getControl() != null)
						...
						case BREAK:
							return null;
						case CONTINUE:
							throw new prisms.lang.EvaluationException("continue outside of loop", pi,
								pi.getMatch().index);
						}
				}
			}
