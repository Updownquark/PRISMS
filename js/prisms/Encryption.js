...
__dojo.provide("prisms.Encryption");

__dojo.require("dojox.encoding.crypto.Blowfish");
__dojo.require("prisms.AESEncryption");
...
prisms.Encryption={
	...
	/**
	 * Creates a cipher to encrypt and decrypt text
	 * 
	 * @param type The type of the cipher to create:
	 * 		* null: Creates a cipher that does not encrypt or decrypt text
	 * 		* dojo-blowfish: Creates a cipher based on dojo's Blowfish implementation. This cipher
	...
	createNullCipher: function(){
		return {
			setParams: function(params){/*Do nothing*/},
			init: function(key){/*Do nothing*/},
	...
	createDojoBlowfish: function(){
		return new prisms.DojoBlowfishCipher();
	},
...
prisms.DojoBlowfishCipher=function()
{
	...
	this.init=function(key){
		...
		for(var k=0;k<keys.length;k++)
			key+=keys[k];
		this._key=key;
		this._blowfishAO={cipherMode: __dojox.encoding.crypto.cipherModes.ECB,
			outputType: __dojox.encoding.crypto.outputTypes.Base64};
	};

	this._to64Str=function(value){
		var ret = "";
		while(value > 0)
		{
	...
	this.encrypt=function(text){
		return __dojox.encoding.crypto.Blowfish.encrypt(text, this._key, this._blowfishAO);
	};

	/**
	 * The decrypt method hasn'd had success decrypting java-encrypted data
	 */
	this.decrypt=function(encrypted){
		return __dojox.encoding.crypto.Blowfish.decrypt(encrypted, this._key, this._blowfishAO);
	};
};
...
prisms.MovableTypeAESCipher=function()
{
	...
	this.setParams=function(params){
		...
		if(m==this.acceptableModes.length)
			throw new Error("Only AES modes supported are: "+__dojo.toJson(this.acceptableModes));
 ...
 *
 * @modified Andrew Butler Modified the way the cipher key is generated
