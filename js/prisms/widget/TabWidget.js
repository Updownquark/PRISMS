...
__dojo.provide("prisms.widget.TabWidget");
__dojo.declare("prisms.widget.TabWidget", [__dijit._Widget, __dijit._Contained], {
	_visible: true,
...
*/	},
	...
	setSelected: function(selected, recursive){
		if(selected)
		{
	//		if(!__dojo.isIE) //Had an issue with this in IE
	...
	getWidgetParent: function(widget){
		...
		while(parentDomNode.parentNode && (parentDomNode.id==null || parentDomNode.id.length==0
			|| !(ret=__dijit.byId(parentDomNode.id))))
