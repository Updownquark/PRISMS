...
__dojo.provide("prisms.widget.LayerConfigurator");
__dojo.declare("prisms.widget.LayerConfigurator", prisms.widget.PrismsDialog, {
	...
	_setupUI: function(){
		...
		__dojo.connect(this.wmsLayerName, "onchange", this, function(){
			if(this.dataLock)
				return;
		...
		__dojo.connect(this.wmsLayerURL, "onchange", this, function(){
			if(this.dataLock)
				return;
	...
	setLayerRow: function(tableRow, layer, isBottom, isTop){
		...
		if(!isBottom)
		{
			...
			img.src="__webContentRoot/rsrc/icons/buttons/movedown.gif";
			tableRow.cells[4].appendChild(img);
			this._listeners.push(__dojo.connect(tableRow.cells[4].childNodes[0], "onclick", this, function(){
		...
		{
			img=document.createElement("img");
			img.src="__webContentRoot/rsrc/icons/buttons/moveup.gif";
			tableRow.cells[5].appendChild(img);
			this._listeners.push(__dojo.connect(tableRow.cells[5].childNodes[0], "onclick", this, function(){
	...
	clearListeners: function(){
		while(this._listeners.length>0)
			__dojo.disconnect(this._listeners.pop());
	},
